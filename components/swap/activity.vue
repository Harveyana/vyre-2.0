<template>
    <div name="fade" class="w-full max-h-[56vh] h-[56vh] flex flex-col items-center justify-center gap-3 ">
      ...
      <LottieLoader v-if="showLoader" />

      <div v-else class="w-full h-full flex flex-col items-start justify-center gap-3">
        ...
        <div v-if="transaction?.transactionStatus === 'AWAITING_TRANSFER'" class="w-full h-full overflow-y-auto flex flex-col items-start justify-center gap-1 sm:gap-y-4 bg-[#FFFFFF] rounded-2xl ">
        ...
        <div v-else class="w-full h-full overflow-y-auto flex flex-col items-start justify-center gap-1 sm:gap-y-2 bg-[#FFFFFF] rounded-2xl ">
            
            <div class="w-full h-full flex flex-col sm:flex-row items-start justify-start gap-2 ">
              <div class="w-full h-full  bg-[#F6F6F6] flex flex-col gap-2 items-center justify-start p-3 rounded-2xl border border-gray-400 border-dashed ">

                <div class="w-full flex flex-col items-center justify-center gap-3">
                  <h1 class="Grotesque-Bold whitespace-nowrap text-[18px] text-black ">
                    Transaction Details
                  </h1>
                ...
                <div class="w-full flex flex-col items-start justify-start py-2">
                  <h1 class="Grotesque-Regular whitespace-nowrap text-[14px] ">
                   *xxxxxxxxxxxxxx{{ transaction?.transactionId.slice(25)}}
                  </h1>
                  <h1 class="Grotesque-Regular whitespace-nowrap text-[14px] ">
                   {{ transaction?.source.sourceCurrency.label}}
                  </h1>
                  <span class="Grotesque-Light  text-[12px] sm:text-[13px] whitespace-nowrap">
                    {{transaction?.source?.sourcePaymentMethod}}
                  </span>
                ...
                <div class="w-full flex items-center justify-between  border-b border-gray-300 py-2">
                  <h1 class="Grotesque-Regular whitespace-nowrap text-[14px] text-black ">
                    Status:
                  </h1>
                  <span :class="transaction?.transactionStatus === 'AWAITING_TRANSFER' ? 'text-[#FFA500]':transaction?.transactionStatus === 'CREATED' ? 'text-[#3498DB]':transaction?.transactionStatus === 'PROCESSING' ? 'text-[#9B59B6]':transaction?.transactionStatus === 'COMPLETED' ? 'text-[#2ECC71]':transaction?.transactionStatus === 'FAILED' ? 'text-[#E74C3C]':'text-green-500'" class="Grotesque-Light  text-[12px] sm:text-[13px] whitespace-nowrap">
                ...
                <div v-if="transaction?.transferInstructions?.type === 'fiat'" class="w-full flex items-center justify-between border-b border-gray-300 py-2 ">
                  <h1 class="Grotesque-Regular whitespace-nowrap text-[14px] text-black ">
                    Bank:
                  </h1>
                  <span class="Grotesque-Bold  text-[12px] sm:text-[13px] whitespace-nowrap">
                ...
                <div v-if="transaction?.transferInstructions?.type === 'crypto'" class="w-full flex items-center justify-between gap-2 border-b border-gray-300 py-2">
                  <span class="w-[80%] Grotesque-Light truncate text-[12px] sm:text-[13px]">
                    {{transaction?.transferInstructions?.transferWalletAddress}}
                  </span>
                ...
                <div class="w-full flex items-center justify-between py-3 px-2">
                  <h1 class="Grotesque-Regular whitespace-nowrap text-[14px] ">
                    AMOUNT
                  </h1>
                  <span class="Grotesque-Light  text-[12px] sm:text-[13px] whitespace-nowrap">
