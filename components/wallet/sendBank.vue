<template>
      ...
      <div  class="w-full border flex flex-col px-4 rounded-2xl sm:shadow-md py-3">
        ...
        <div class="w-full flex items-start justify-between sm:justify-center sm:gap-x-16 border px-4 rounded-2xl py-4">
          ...
          <div class="w-full flex flex-col items-start justify-center gap-y-2">
            ...
            <input  v-model="BANK.accountNumber" class="w-full bg-gray-200 outline-none rounded-xl focus-none border-none text-black Grotesque-Bold text-[14px] py-2 px-3" name="numberInput" placeholder="Account Number" />
            <input  v-model="BANK.accountName" class="w-full bg-gray-200 outline-none rounded-xl focus-none border-none text-black Grotesque-Bold text-[14px] py-2 px-3" name="numberInput" placeholder="Account Name" />
  ...
  const submitInfo = ()=>{
    if(!BANK.accountName || !BANK.accountNumber || !BANK.name) return
    emit('update-value', BANK)
  }
