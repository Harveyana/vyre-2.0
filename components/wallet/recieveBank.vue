<template>
      ...
      <div  class="w-full border flex flex-col px-4 rounded-2xl sm:shadow-md py-3">
        ...
        <div class="w-full flex items-start justify-between sm:justify-center sm:gap-x-16 border px-4 sm:px-8 rounded-2xl py-4">
          ...
          <div class="flex flex-col items-start justify-center">
            ...
            <input v-model="entry" class="w-36 bg-gray-200 sm:w-60 outline-none rounded-xl focus-none border-none text-black Grotesque-Bold text-2xl"  type="number" id="numberInput" name="numberInput" pattern="[0-9]*" inputmode="numeric" />
  ...
  const { initDeposit, fetchRate} = useWalletStore();
  ...
  const initialize = async()=>{
      ...
      toast.promise(() => new Promise(async(resolve,reject) =>{
        const init = await initDeposit(currency!, entry.value)
        if (init.success) {
          resolve({msg:init.msg, url:init?.payment?.url})
      ...
      {
        loading: 'Redirecting to payment url',
