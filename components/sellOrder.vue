      ...
      <div  class="w-full rounded-2xl flex flex-col items-center justify-center">
        ...
        <div class="w-full sm:w-fit sm:max-w-[90%] flex flex-col px-4 rounded-2xl sm:shadow-md py-3 ">
          ...
          <div class="w-full flex flex-col sm:flex-row-reverse items-start justify-between sm:justify-center gap-x-4 gap-y-4 sm:gap-x-10 border px-4 sm:px-8 rounded-2xl py-4">
            ...
            <div class="sm:w-[25%] flex flex-col ml-auto items-center justify-center">
            ...
            <div class="w-full sm:w-[73%] flex flex-col items-start jhustify-center gap-y-4 sm:gap-y-2">

              <div
                class="rounded-2xl w-full sm:w-fit h-fit flex flex-col items-start justify-center text-grass11 bg-[#E5DEF0] py-1 px-4 sm:px-2 gap-x-1 sm:gap-x-2 outline-none"
                aria-label="dropdown menu options"
              >
                <!-- Enter Amount to sell -->
                <div class="flex items-center justify-center cursor-pointer text-white gap-2">
                  <img :src="currentBase.img" class="w-7 sm:w-8 bg-[#bfcbdf] rounded-full border-2 border-white"/>
                      
                  <input v-model="order.amount" placeholder="Amount" class="w-full sm:w-48 bg-[#E5DEF0] outline-none focus-none text-black poppinsRegular text-xl sm:text-2xl"  type="number" id="numberInput" name="numberInput" pattern="[0-9]*" inputmode="numeric" />
                </div>
                ...
                <!-- enter price to sell at -->
                <div class="flex items-center justify-center cursor-pointer text-white gap-2">
                  <img :src="currentQuote.img" class="w-7 sm:w-8 bg-[#bfcbdf] rounded-full border-2 border-white"/>
                      
                  <input v-model="order.price" :placeholder="Math.round(currentRate)" class="w-full sm:w-48 bg-[#E5DEF0] outline-none focus-none text-black poppinsRegular text-xl sm:text-2xl"  type="number" id="numberInput" name="numberInput" pattern="[0-9]*" inputmode="numeric" />
                </div>
          ...
          <div class="w-full flex items-center justify-between mt-2 px-3">
            ...
            <span class="text-xs sm:text-sm truncate poppinsRegular ml-4">
                Balance ~ {{currentBase.symbol}} {{currentBase.balance}}
            </span>
...
<script setup lang="ts">
  ...
  // initToast(false,'Experience Required','Your work experience is required')
    ...
    const updateEntry = ()=>{
      order.amount = parseFloat((sliderValue.value[0]/100 * currentBase.value.balance).toFixed(8))
    }
