      ...
      <div  class="col-span-12 rounded-2xl flex flex-col items-center justify-center">
                ...
                <DropdownMenuRoot v-model:open="toggleState">
                  ...
                  >
                    <div @click="toggle" class="flex items-center justify-center cursor-pointer text-white gap-2">
                      <img :src="assetValue.img" class="w-7 sm:w-8 bg-[#bfcbdf] rounded-full border-2 border-white"/>
        ...
        <div v-if="tab == 'tab2'" class="w-full sm:w-fit flex flex-col px-4 rounded-2xl sm:shadow-md py-3">
          ...
          <div class="w-full flex flex-col items-center justify-center sm:gap-y-8 border px-4 sm:px-8 rounded-2xl py-4">
            ...
            <!-- For vyre to vyre transfers -->
            <div v-if="props.mode == 'vyre'" class="w-full flex flex-col items-start justify-center">
              <span class="text-sm truncate poppinsRegular ml-2">
                vyre email
              </span>
              <input v-model="emailReceipient" class="w-full p-2.5 outline-none focus-none text-black poppinsRegular text-sm bg-gray-200 rounded-lg" type="text" id="numberInput" placeholder="Enter vyre email" />

              <div v-if="emailReceipient !== ''" class="w-full py-1">

                <div v-if="!userloading" class="w-full flex mt-2">
                  <img src="~/assets/img/boy.png" class="w-7 sm:w-8 bg-[#bfcbdf] rounded-full border-2 border-white"/>
                  <div class="flex flex-col">
                    <h4 class="text-xs truncate poppinsRegular ml-2">
                      Harvey Ana
                    </h4>
            ...
            <!-- for blockchain tranfers -->
            <div v-if="props.mode == 'blockchain'" class="w-full flex flex-col items-start justify-center">
             
             <input v-model="emailReceipient" class="w-full p-2.5 outline-none focus-none text-black poppinsRegular text-sm bg-gray-200 rounded-lg" type="text" id="numberInput" placeholder="Enter blockchain address" />
             <span class="text-sm truncate poppinsRegular ml-2">
               Blockchain Address
             </span>
            ...
            <!-- for bank tranfers -->
            <div v-if="props.mode == 'bank'" class="w-full flex flex-col items-start justify-center border p-4 rounded-2xl">
             
             <div class="w-full flex flex-col w-max-[70%] gap-y-1" >
              <span class="text-sm truncate poppinsRegular ml-2">
               Account No:
              </span>
              <input v-model="bankdetails.AccNumber" class="w-[70%] p-2.5 outline-none focus-none text-black poppinsRegular text-sm bg-gray-200 rounded-lg" type="text" id="numberInput" placeholder="Bank account number" />
             </div>
             ...
             <div class="w-full flex flex-col w-max-[70%] gap-y-1" >
              ...
              <input v-model="bankdetails.AccName" class="  p-2.5 outline-none focus-none text-black poppinsRegular text-sm bg-gray-200 rounded-lg" type="text" id="numberInput" placeholder="Bank account name" />
             </div>
             ...
             <div class="w-full flex flex-col w-max-[70%] gap-y-1" >
              ...
              <input v-model="bankdetails.bankName" class="  p-2.5 outline-none focus-none text-black poppinsRegular text-sm bg-gray-200 rounded-lg" type="text" id="numberInput" placeholder="Bank name" />
             </div>
...
definePageMeta({
  layout: 'action'
   ...
   // Receipient 
    ...
    const assetValue = ref(AssetMap[name.value]);
    const assets = computed(() => Object.values(AssetMap).filter(asset => asset.type === props.type))

    const switchAsset = (name:string)=>{
      assetValue.value = AssetMap[name]
    }
