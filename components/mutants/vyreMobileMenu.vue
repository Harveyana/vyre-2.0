<template>
   <div class="relative flex w-fit top-0 left-[1px] flex sm:hidden items-center justify-start rounded-3xl bg-black px-2 py-2 gap-3">
     <!-- Menu Button -->
     <button 
     ...
     <transition name="fade">
       <ul 
         ...
         class="absolute left-20 mt-0 flex items-center justify-between gap-2 bg-black rounded-3xl p-2 z-50 shadow-lg"
       >
         <li>
           <NuxtLink 
             href="/" 
         ...
         <li>
           <NuxtLink 
             href="/receive" 
             :class="$route.path == '/receive' ? 'text-black bg-white' : 'text-white bg-black hover:bg-white dark:hover:bg-gray-700'"  
             class="flex items-center justify-center gap-1 py-2 px-3 text-base rounded-full group"
             @click="closeMenu"
           >
             <svg xmlns="http://www.w3.org/2000/svg" :class="$route.path == '/receive' ? 'text-black' : 'text-gray-500'" class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 group-hover:text-black" viewBox="0 0 512 512">
