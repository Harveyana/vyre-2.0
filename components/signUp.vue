<template>
      <div class="w-full h-fit sm:w-[60%] lg:w-[75%] space-y-8 my-8 sm:space-y-4 overflow-y-scroll no-scrollbar">
        ...
        <div class="w-full flex flex-col items-start justify-start space-y-5 ">
          <h1 v-if="showLoader == false" class=" text-4xl text-white font-bold" data-aos="fade-right" data-aos-once="true">
            Sign Up
          </h1>
          ...
          <form class="w-full flex flex-col items-start justify-start space-y-4 ">
            ...
            <div class="w-full flex flex-col items-start justify-start space-y-1">
              ...
              <input
                v-model="inputs.name"
                type="text"
                class="w-full border-[#555a5c] border text-gray-400 text-[13px] py-2 px-4 bg-[#12171d] rounded-3xl outline-[#89ead9]"
            ...
            <div class="w-full flex flex-col items-start justify-start space-y-1">
              ...
              <div class="w-full flex items-center justify-center border-[#555a5c] border bg-[#12171d] rounded-3xl">
                <input
                  ...
                  class="w-full text-gray-400 text-[13px] py-2 px-4 bg-[#12171d] rounded-3xl outline-[#89ead9]"
            ...
            <!-- <div class="w-full flex flex-col items-start "> -->
              <button :disabled="!isValidForm" @click.prevent="submitForm" @keyup.enter="submitForm" :class="{ 'bg-[#34da97]': isValidForm }" class="w-full bg-[#89ead9] flex flex-row items-center justify-center rounded-3xl py-2">
                <span v-if="!showLoader" class="text-[14px] text-black hover:text-gray-200 text-center">Create Account</span>
 ...
 const isValidForm = computed(() =>{ return inputs.name !== '' && isValidEmail(inputs.email) && inputs.password.length >= 6;})
  ...
  const Signup = async(name:string,email:string,password:string)=>{   
    ...
    const created = await createUser(inputs);

    if (created) {
          showLoader.value = false
          initToast(true,'Successful','Account created successful')
          inputs.name = '';
          inputs.email = '';
          inputs.password = '';

          route.push('/kyc'); // route to the dashboard
    }else{
          showLoader.value = false
