<template>
  <div class="w-full h-full flex justify-center items-start bg-white/60 rounded-2xl sm:p-3">
    <!-- Loading State -->
    <div v-if="loading" class="flex flex-col gap-y-4 w-full rounded-xl items-center justify-center">

      <svg class="animate-pulse" width="280" height="280" viewBox="0 0 280 280" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M52.6631 127.029C52.6631 127.029 91.434 125.477 99.8125 61.7456C107.253 5.14629 172.631 30.9083 191.529 53.4701C216.01 82.6971 202.96 132.974 234.629 139.325C266.299 145.676 245.618 216.292 197.546 209.533C137.746 201.125 152.214 236.069 134.262 250.859C121.379 261.475 68.5515 251.256 66.7908 212.723C65.3092 180.297 51.6315 180.4 41.9366 176.723C27.9544 171.421 19.1441 133.017 52.6631 127.029Z" fill="#F1F3F7"/>
        <path d="M184.462 197.807L146.371 160.168L138.828 167.801L176.92 205.44L184.462 197.807Z" fill="#96CCB5"/>
        <path d="M199.013 207.92L169.378 178.637C169.213 178.474 168.947 178.476 168.784 178.641L157.568 189.992C157.405 190.157 157.406 190.423 157.571 190.586L187.206 219.869C187.371 220.032 187.637 220.031 187.8 219.866L199.017 208.514C199.18 208.349 199.178 208.083 199.013 207.92Z" fill="#34DA97"/>
        <path d="M112.611 179.097C138.816 179.097 160.06 157.853 160.06 131.648C160.06 105.442 138.816 84.1982 112.611 84.1982C86.4051 84.1982 65.1613 105.442 65.1613 131.648C65.1613 157.853 86.4051 179.097 112.611 179.097Z" fill="#34DA97"/>
        <path d="M112.611 171.355C134.54 171.355 152.318 153.578 152.318 131.648C152.318 109.718 134.54 91.9407 112.611 91.9407C90.6811 91.9407 72.9036 109.718 72.9036 131.648C72.9036 153.578 90.6811 171.355 112.611 171.355Z" fill="#D9E6FA"/>
        <path d="M192.83 88.7531C181.7 85.1454 169.672 91.247 166.049 102.423C162.442 113.553 168.543 125.581 179.719 129.203C190.895 132.825 202.892 126.664 206.5 115.533C210.108 104.403 203.96 92.3606 192.83 88.7531ZM189.471 99.1145C190.717 99.5183 191.815 100.283 192.626 101.312C193.436 102.34 193.923 103.587 194.024 104.892C194.126 106.198 193.837 107.504 193.195 108.646C192.552 109.787 191.586 110.712 190.417 111.303C189.249 111.894 187.931 112.125 186.631 111.966C185.331 111.807 184.107 111.266 183.115 110.411C182.124 109.556 181.408 108.425 181.06 107.163C180.711 105.9 180.746 104.563 181.158 103.32C182.273 99.8813 186.032 97.9999 189.471 99.1145ZM181.068 125.041C177.37 123.848 174.202 121.407 172.106 118.136C176.029 115.907 180.13 115.286 183.84 116.489C187.55 117.692 190.567 120.569 192.377 124.706C188.76 126.126 184.762 126.244 181.068 125.041Z" fill="#96CCB5"/>
        <path d="M100.191 190.176C98.258 191.163 96.6608 192.701 95.6016 194.595C94.5424 196.49 94.0688 198.656 94.2406 200.82C94.4125 202.983 95.2221 205.047 96.5671 206.751C97.9121 208.455 99.7321 209.721 101.797 210.39C103.862 211.06 106.078 211.101 108.167 210.511C110.256 209.92 112.122 208.723 113.531 207.072C114.939 205.42 115.826 203.388 116.08 201.233C116.333 199.077 115.942 196.895 114.955 194.961C113.629 192.371 111.331 190.413 108.563 189.516C105.795 188.618 102.784 188.856 100.191 190.176ZM102.747 195.184C103.349 194.876 104.029 194.755 104.701 194.834C105.372 194.914 106.005 195.191 106.518 195.63C107.032 196.07 107.403 196.652 107.585 197.303C107.768 197.954 107.752 198.645 107.541 199.287C107.331 199.93 106.934 200.495 106.401 200.911C105.869 201.328 105.225 201.577 104.55 201.626C103.876 201.676 103.202 201.525 102.614 201.191C102.027 200.857 101.551 200.356 101.248 199.752C100.847 198.946 100.779 198.015 101.06 197.16C101.34 196.305 101.947 195.594 102.747 195.183V195.184ZM109.143 207.714C107.358 208.628 105.311 208.893 103.352 208.464C103.97 206.219 105.24 204.496 107.034 203.581C108.827 202.665 110.978 202.615 113.149 203.464C112.347 205.302 110.931 206.804 109.143 207.713V207.714Z" fill="#96CCB5"/>
      </svg>

    </div>

    <!-- Content -->
    <div v-else class="w-full flex flex-col items-start justify-center gap-y-4 sm:mt-16">


      <div class="w-full flex flex-col gap-2 px-3">
        <div class="flex justify-between items-center w-full">
          <span class="text-sm font-medium text-gray-700 ">
            Order Type
          </span>
        </div>

        <TabsRoot v-model:modelValue="entry.type" class="w-[40%] sm:w-[50%] bg-[#F8F8FC] h-fit rounded-2xl" default-value="BUY">
              <TabsList class="h-full p-1 rounded-2xl bg-[#F8F8FC] shrink-0 flex items-center justify-center" aria-label="Manage your account">

                <TabsTrigger
                      class="Grotesque-Regular py-2.5 flex-1 flex items-center justify-center text-[14px] leading-none text-[#A7A7A7] select-none  rounded-2xl  data-[state=active]:bg-white data-[state=active]:text-black outline-none cursor-default transition ease-in-out duration-300"                      
                      value="BUY"
                    >
                    Buy
                </TabsTrigger>

                <TabsTrigger
                      class="Grotesque-Regular py-2.5 flex-1 flex items-center justify-center text-[14px] leading-none text-[#A7A7A7] select-none  rounded-2xl  data-[state=active]:bg-white data-[state=active]:text-black outline-none cursor-default transition ease-in-out duration-300"                      
                      value="SELL"
                    >
                    Sell
                </TabsTrigger>
                    
              </TabsList>
                        
        </TabsRoot>

      </div>

      <div class="w-full flex flex-col gap-2 px-3">
        <div class="flex justify-between items-center w-full">
          <span class="text-sm font-medium text-gray-700 ">
            Order Pair
          </span>
        </div>

        <div class="w-full flex gap-x-2 overflow-x-auto items-center justify-start py-2 px-3">
          <button
            v-for="pair in allPairs"
            :key="pair?.id"
            @click="selectPair(pair)"
            class="border rounded-3xl px-3 py-1 transition-colors duration-200 group"
            :class="{
              'border-black hover:bg-gray-100': selectedPair?.id === pair?.id,
              'border-none': selectedPair?.id !== pair?.id
            }"
          >

            <div class="flex items-center justift-center">
              <!-- <img :src="order?.pair?.baseWallet?.imgurl" class="z-10 w-10 sm:w-12 -mr-2 sm:-mr-2.5  rounded-full"/> -->
              <div class="z-10 flex items-center justify-center">
                <div v-if="pair?.baseCurrency && pair?.baseCurrency.type ==='CRYPTO'" class="flex items-end justify-center">
                 <img class="w-[40px] sm:w-[40px] bg-white rounded-full" :src="pair?.baseCurrency?.imgUrl" alt="avatar">
                 <img v-if="pair?.baseCurrency?.isStablecoin" class="w-[20px] bg-black rounded-full -ml-5" :src="pair?.baseCurrency?.chainImgUrl" alt="avatar">
                </div>
                <h3 v-else class="text-white text-[30px] bg-white rounded-full  ">{{ pair?.baseCurrency?.flagEmoji }}</h3>
              </div>


              <!-- <img :src="order?.pair?.quoteWallet?.imgurl" class="z-20 w-6 sm:w-8 rounded-full self-end"/> -->

              <div class="z-20 flex items-center justify-center self-end">
                <div v-if="pair?.quoteCurrency && pair?.quoteCurrency.type ==='CRYPTO'" class="flex items-end justify-center">
                 <img class="w-[30px] rounded-full" :src="pair?.quoteCurrency?.imgUrl" alt="avatar">
                 <img v-if="pair?.quoteCurrency?.chainImgUrl" class="w-[20px] bg-black rounded-full -ml-2" :src="pair?.quoteCurrency?.chainImgUrl" alt="avatar">
                </div>
                <h3 v-else class="text-white text-[30px] ">{{ pair?.quoteCurrency?.flagEmoji }}</h3>
              </div>

            </div>

            <span 
              class="whitespace-nowrap font-200 text-[13px] transition-colors duration-200"
              :class="{
                'text-black Grotesque-SemiBold group-hover:text-[#34da97]': selectedPair?.id === pair?.id,
                'text-black Grotesque-Regular': selectedPair?.id !== pair?.id
              }"
            >
              {{pair?.name}}

            </span>
          </button>
        </div>

      </div>

      <!-- Full-width Price Range Filter -->
      <!-- <div class="w-full flex flex-col gap-2 px-3">
        <div class="flex justify-between items-center w-full">
          <span class="text-sm font-medium text-gray-700">
            Price Range
          </span>
          <span class="text-sm text-gray-700 ">
            {{ sliderValue[0] }} - {{ sliderValue[1] }}
          </span>
        </div>
        
        <div class="w-full px-2">
          <SliderRoot
            v-model="sliderValue"
            class="relative flex items-center select-none touch-none w-full h-5"
            :max="priceRange.max"
            :min="priceRange.min"
            :step="priceStep"
          >
            <SliderTrack class="relative grow rounded-full h-[3px] bg-gray-200 w-full">
              <SliderRange class="absolute bg-[#34da97] rounded-full h-full" />
            </SliderTrack>
            <SliderThumb
              class="block w-4 h-4 bg-white rounded-full border-2 border-[#34da97] focus:outline-none focus:shadow-[0_0_0_3px] focus:shadow-black/20"
            />
            <SliderThumb
              class="block w-4 h-4 bg-white rounded-full border-2 border-[#34da97] focus:outline-none focus:shadow-[0_0_0_3px] focus:shadow-black/20"
            />
          </SliderRoot>
        </div>
      </div> -->

      <div class="w-full flex items-center justify-center gap-3 px-3">
        <!-- Min Price Input -->
        <div class="flex flex-col justify-center items-start w-1/2">
          <span class="text-sm font-medium text-gray-700 ">
            Min
          </span>
          <input
            v-model="formattedMin"
            @blur="onBlur('priceMin')"
            type="text"
            class="w-full bg-gray-200 rounded-xl px-2 py-3 border-0 outline-none focus:outline-none focus:ring-0 focus:ring-offset-0 focus:border-0"
          />
        </div>

        <!-- Max Price Input -->
        <div class="flex flex-col justify-center items-start w-1/2">
          <span class="text-sm font-medium text-gray-700 ">
            Max
          </span>
          <input
            v-model="formattedMax"
            @blur="onBlur('priceMax')"
            type="text"
            class="w-full bg-gray-200 rounded-xl px-2 py-3 border-0 outline-none focus:outline-none focus:ring-0 focus:ring-offset-0 focus:border-0"
          />
        </div>
      </div>

      <button @click="update()" class="w-full hover:bg-black/80 bg-black items-center justify-center rounded-2xl py-3 mt-4">
        <span class="text-[16px] font-medium text-gray-300">
            Apply
        </span>
      </button>

    </div>

  </div>
</template>

<script setup lang="ts">

import { storeToRefs } from 'pinia'
import { useOrderStore } from '~/store/order'

const { fetchPairs } = useOrderStore()
// const { loading } = storeToRefs(useOrderStore())

const emit = defineEmits(['update'])  // Declare Events

// Data
const loading = ref(false)
const allPairs = ref<any[]>([])
const selectedPair = ref<any>()
const priceRange = ref({
  min: 1,
  max: 1000
})
// const priceStep = 10
// const sliderValue = ref([priceRange.value.min, priceRange.value.max])



// Format number with commas
const formatNumber = (value: number | null | string ): string => {
  if (value === null) return ''
  return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
}

// Parse number by removing commas
const parseNumber = (value: string): number | null => {
  const num = value.replace(/,/g, '')
  return num ? parseFloat(num) : null
}

const price = reactive({
  priceMin: null,
  priceMax: null
});

// Computed properties for formatted values
const formattedMin = computed({
  get(): string {
    return formatNumber(price.priceMin)
  },
  set(value: string) {
    price.priceMin = parseNumber(value)
  }
})

const formattedMax = computed({
  get(): string {
    return formatNumber(price.priceMax)
  },
  set(value: string) {
    price.priceMax = parseNumber(value)
  }
})

const entry = reactive({
  cursor:'',
  priceMin: formattedMin.value,
  priceMax: formattedMax.value,
  type:'BUY',
  pairId: selectedPair.value?.id
})

// const entry = computed(() => ({
//   cursor: '',
//   priceMin: price.priceMin,
//   priceMax: price.priceMax,
//   type: 'BUY',
//   pairId: selectedPair.value?.id
// }))



// Reformat on blur to ensure proper comma formatting
const onBlur = (field: 'priceMin' | 'priceMax') => {
  const currentValue = price[field]
  if (currentValue !== null) {
    price[field] = parseNumber(currentValue.toString())
  }
}

const selectPair = (value: any) => {
  selectedPair.value = value
  entry.pairId = value?.id
}

// watch(() =>[ entry.type, sliderValue.value[0], sliderValue.value[1], entry.pairId], async(newvalue, oldvalue) => {
  
// });


const update = ()=>{
  console.log(entry)
  emit('update',entry)
}


const fetchData = async () => {
  loading.value = true
  const result = await fetchPairs()
  
  if (result?.success) {
    allPairs.value = result.pairs || []
    if (allPairs.value.length > 0) {
      selectedPair.value = allPairs.value[0]
      entry.pairId = allPairs.value[0]?.id
    }
  }
  loading.value = false
}


onMounted(async () => {
  await fetchData()
})

</script>

<style scoped>
  /* Full-width slider track */
  /* [data-radix-slider-track] {
    width: 100%;
  } */

  /* Thumb focus styles */
  /* [data-radix-slider-thumb]:focus {
    outline: none;
    box-shadow: 0 0 0 3px rgba(52, 218, 151, 0.3);
  } */
</style>