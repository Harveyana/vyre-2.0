<template>
        ...
        <div class="flex flex-col w-full sm:w-[75%] lg:w-[55%] sm:py-12 p-4 sm:px-10 h-full gap-y-6 my-4">
          ...
          >
            <div class="w-full relative">
              ...
              <input id="place" v-model="sender.location" 
            ...
            <div class="w-full flex flex-row items-center justify-start">
              ...
              <div v-else class="w-full flex flex-col items-start justify-center gap-y-1">
                ...
                <input 
                  id="place" @input="" 
                  v-model="sender.additional_Number" 
                  v-mask="'+234 ##########'" 
                  placeholder="+234 810000000" 
                  class="w-full bg-[#F9F9FC] rounded-lg border border-gray-300 px-3 py-3 text-[12px] text-gray-800 outline-black">
                </input>
  ...
  const submitForm = ()=>{
    console.log(sender)
    if(!sender.location || !sender.name || !sender.phoneNumber ) return
  ...
  onMounted(async () => {
    ...
    autocomplete.addListener('place_changed', () => {
        ...
        sender.longitude = place.geometry.location.lng()
        sender.latitude = place.geometry.location.lat()

        sender.location = joinAddressComponents(place.address_components)
        return
    });
